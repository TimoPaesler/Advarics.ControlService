...
namespace Advarics.ControlService
{
	...
	public class AdvaricsControlService : IAdvaricsControlService
	{
		...
		public VersionInfoResponse GetVersions(VersionRequest request)
		{
			...
			if (request.IdToken == SecurityIdProvider.GetIDToken())
			{
				...
				try
				{
					...
					{
						response.RetailPakNo = updateStatus.CurrentUpdatePackage.ToString();
					...
					try
					{
						...
						if (xmlRental != string.Empty)
						{
							...
							{
								response.RentalServicePakNo = item.PackageNo.ToString();
							}
						}
						if (xmlSecuDataService != string.Empty)
						{
							...
							{
								response.SecuDataServicePakNo = item.PackageNo.ToString();
							}
						}
						if (xmlStatisticService != string.Empty)
						{
							...
							{
								response.StatisticServicePakNo = item.PackageNo.ToString();
							}
						}
						if (xmlWebShopService != string.Empty)
						{
							...
							{
								response.WebShopServicePakNo = item.PackageNo.ToString();
							}
						}
					}
					catch
		...
		public UpdateInfoResponse UpdateCustomer(UpdateRequest request)
		{
			...
			//Hole den richtigen Api-Namen --> bezieht sich auf den Ordner, der zweite Teil hinter retail_
			//var externalApiName = request.Customer.Name + ".api.external.core";
			var customerNameRetail = request.Customer.BaseCustomerPath.Replace("retail_", "");

			var externalApiName = customerNameRetail + ".api.external.core";

			logResponse.Add(Log.LogEntry(DateTime.Now, "-", "-", null, "Debug only : " + externalApiName + " Package No " + request.PackageNo, "0", false, false, DateTime.Now, 0, "Server", "-", Log.LogStatus.OK));


			var appPool = new ServerManager().ApplicationPools[externalApiName]; //KUNDENNAME.api.external.core
			...
			if (request.IdToken == SecurityIdProvider.GetIDToken())
			{
				...
				if (appPool != null)
				{
					...
					{
						logResponse.Add(Log.LogEntry(DateTime.Now, "-", "-", null, "External-Api already stopped : " + externalApiName, "0", false, false, DateTime.Now, 0, "Server", "-", Log.LogStatus.OK));
						appPoolStopped = true;
					}
					else
					{
						//// Vor dem Kopieren Beenden der laufenden external Api
						logResponse.Add(Log.LogEntry(DateTime.Now, "-", "-", null, "Stop : " + externalApiName, "0", false, false, DateTime.Now, 0, "Server", "-", Log.LogStatus.OK));
						appPool.Stop();
					}
				}
				...
				try
				{
					...
					try
					{
						...
						DeleteOldDlls.DeleteJS(request.AdvProgramPath + "\\" + request.Customer.BaseCustomerPath + "\\Advarics.Retail\\webapp");
					...
					logResponse.Add(Log.LogEntry(DateTime.Now, "-", "-", null, "xsd-Files are ready", "0", false, true, DateTime.Now, 0, "Server", "-", Log.LogStatus.OK));
					...
					// Delete old dlls auskommentiert am 29.05.2019 wird nicht mehr benötigt und führt zu Fehlern im Log
					//try
					//{
			...
			//Aktuell nur der external api AppPool und der darf nicht gestartet werden wenn er beendet war

			if (appPool != null && appPoolStopped != true)
			{
				logResponse.Add(Log.LogEntry(DateTime.Now, "-", "-", null, "Start : " + externalApiName, "0", false, false, DateTime.Now, 0, "Server", "-", Log.LogStatus.OK));
				appPool.Start();
			}
